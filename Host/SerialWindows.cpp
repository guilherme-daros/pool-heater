...
void SerialWindows::Initialize(const char *sPort, unsigned long BaudRate) {
  handler = CreateFile(sPort, GENERIC_READ, NULL, NULL, OPEN_EXISTING,
  ...
  if (!GetCommState(handler, &sParams)) {
    std::cout << "ERROR!::failed to get current serialWindows parameters"
              << std::endl;
    return;
  }
  ...
  if (!SetCommState(handler, &sParams)) {
    std::cout << "ALERT!:Failed to set THE SerialWindows port parameters"
              << std::endl;
    return;
  }
...
bool SerialWindows::isConnected() { return isConnect; }
